...
- [x] T048 [P] Reproducibility test: compile twice → byte-identical blob + hash in `tools/asset-compiler/tests/reproducible.rs` (Principle XI/III)
...
- [x] T051 [P] Guard test: no runtime SVG/PNG parser is linked into `kivori-assets`/firmware runtime deps in `crates/kivori-assets/tests/no_runtime_svg.rs` (FR-021)
...
- [x] T061 [P] [US4] Frontend tests: control behavior, deterministic 33.333 ms step, axe a11y, and canvas-blits-verbatim in `apps/desktop/src/features/device-studio/__tests__/studio.test.tsx` (SC-011, FR-018)
...
- [x] T074 [US2] Wire the main run loop / embassy tasks binding hardware adapters to the host-tested core in `firmware/esp32-c3/src/main.rs` (constraint 4) — **DONE.** The production loop lives in `runtime.rs` as `Runtime::step` + `run`, written against the three ports only, and `main.rs` binds the real adapters to it. One tick: finish boot once (`booting` → `offline`, FR-014/015); drain inbound bytes through the real decoder, sequence policy, and `Dispatcher`; render on a 33 ms cadence flushing only changed tiles (FR-013), invalidating the cache on a state change; emit at most one safe `Diagnostic`; emit `Health` every 1000 ms. It never returns and never panics on bad input — a transport failure becomes a `LinkLost` diagnostic plus an `offline` transition, so a new `Hello` can restore the session. A plain blocking loop was chosen over embassy tasks: the work is strictly sequential and single-priority, so an executor would add a dependency and a scheduler without removing any wait. Proven by **10 host tests** (`tests/production_runtime.rs`): boot→offline with six tiles, unchanged frame flushing nothing, handshake with nonce echo, Ping/Pong, SetState applied+reported+repainted, malformed frame dropped→safe diagnostic→next valid frame served, duplicate sequence not re-applied, `Bye` dropping to offline, health cadence, and a 40-tick soak. **Residual physical fact:** the `embedded`-only binary brings up clock and serial and then deliberately stops short of the render loop, because entering it requires a panel controller, pin map, and offsets nobody has measured — see T070
