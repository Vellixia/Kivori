...
- [ ] T093 [MANUAL] Validate that hiding/closing the window leaves the device heartbeat + synchronization active, and reopening restores the UI; record the result in `docs/validation-checklist.md` (FR-030, SC-007) — **RETRIED 2026-08-04, STILL BLOCKED.** macOS Automation/AppleEvents is now GRANTED (`tell application "System Events" to count processes` returned 88), but **Accessibility (assistive access) is DENIED**: `System Events got an error: osascript is not allowed assistive access. (-25211)`. Automation alone can enumerate processes but cannot read or drive window and menu-bar UI elements. Evidence: `docs/evidence/t093-attempt-20260804T114632.log` — the real Tauri binary launched (pid alive 6 s, 22 OS threads, listed as a non-background-only GUI process), so assertion 1 of 14 passes and assertion 8 is partial; the 11 distinguishing lifecycle assertions (Device Studio states, play/pause/scrub/step, close-hides-window, process-survives-close, tray Open restores the same instance, webview reload preserves native state, explicit Quit) all require Accessibility. Assertions 12 and 14 are covered only by HOST-class checks, which cannot satisfy a DESKTOP-MANUAL requirement. **EXACT UNBLOCK**: System Settings → Privacy & Security → Accessibility → enable the terminal running the agent (and/or `/usr/bin/osascript`), then re-run
...
- [x] T138 Record the Kivori Spec Kit quality-gate extension as project tooling outside Feature 001's specified scope, or justify its inclusion, per plan/tasks traceability (unrequested) — **DONE.** `docs/spec-kit-gates.md` records the extension as repository-wide developer tooling with its own lifecycle and semantic version, independent of Feature 001 acceptance: no FR, SC, plan touch-point, or feature task depends on it. It also names which paths are installer-generated (`.specify/extensions/`, `.specify/extensions.yml`, `.claude/skills/speckit-kivori-gates-*`) versus the version-controlled source (`tools/spec-kit-extensions/kivori-gates/`), and notes the installer catalog cache is gitignored. Nothing was deleted
